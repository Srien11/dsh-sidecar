...
每个追问都在独立、持久化的 child Session 中进行，完整继承原回答结束前的上下文，却不会跳转、复制或污染主对话。你可以同时保留多个探索分支，随时关闭、恢复、重命名或归档。

## 它能解决什么

- **不中断主线**：验证细节、解释代码或探索替代方案时，主对话保持原样。
- **精确追问**：准确保留选中文字中的标点、换行和代码，避免重新复制粘贴。
- **持续分支**：侧边对话自动保存，同一回答可维护多个独立追问方向。
- **原生融合**：跟随 Harness 的中英文界面和明暗主题，并沿用原有权限边界。

`选中回答片段 → 点击选区旁“追问” → 在右侧继续对话 → 关闭后可随时恢复`

> 当前状态：`0.1.0-beta.0`，面向 `@deepseek-ai/dsh@0.1.0-rc.6`。核心运行路径曾通过真实安装与浏览器冒烟；当前自动化回归包含 87 项测试，并通过类型检查、构建和 Bundle（产物包）契约验证。Harness 仍处于预览期，后续 RC 可能带来破坏性变化。

## 已实现

- 选中当前 Assistant 回答中的文字后，立即在选区旁显示浮动“追问”按钮；回答末尾仍保留整条追问入口和已有分支数量。
- 只接受当前回答内部的有效选区，并精确保留标点、换行和代码；child 仍继承该回答结束前的完整上下文。
...
1. 打开包含已完成 AI 回答的会话。
2. 如需针对具体片段，选中当前回答中的文字，然后点击选区旁立即出现的“追问”。
3. 如未选择文字，可点击回答尾部的“追问”，针对整条回答继续提问。
...
当前 87 项测试覆盖锚点存储/RPC、sidecar 身份与归档边界、fork 原子性、即时精确选区、多分支管理、焦点、双语词典、主题契约和 child transcript 投影。
